...
class DepthVideo:
    def __init__(self, image_size=[480, 640], buffer=1024, stereo=False, device="cuda:0", 
                 opt_intr=True, camera_model="pinhole"):
        ...
        if camera_model == "pinhole":
            ...
            self.n_intr = 5
            self.model_id = 1
        elif camera_model == "focal": # (f, f, cx, cy) but only estimate f
            self.n_intr = 4
            self.model_id = 2
        else:
            raise Exception("Camera model not implemented! Choose either pinhole, focal or mei model.")
        ...
        self.intrinsics = torch.zeros(buffer, self.n_intr, device="cuda", dtype=torch.float).share_memory_()

        self.stereo = stereo
        c = 1 if not self.stereo else 2

        self.opt_intr = opt_intr
    ...
    def distance(self, ii=None, jj=None, beta=0.3, bidirectional=True):
        ...
        if bidirectional:
            ...
            d = droid_backends.frame_distance(
                self.poses, self.disps, self.intrinsics[0], ii, jj, beta, self.model_id)
    ...
    def ba(self, target, weight, eta, ii, jj, t0=1, t1=None, itrs=2, lm=1e-4, ep=0.1, 
        ...
        with self.get_lock():
            ...
            droid_backends.ba(self.poses, self.disps, self.intrinsics[0], self.disps_sens,
                    target, weight, eta, ii, jj, t0, t1, itrs, self.model_id, lm, ep, 
                    motion_only, self.opt_intr)
