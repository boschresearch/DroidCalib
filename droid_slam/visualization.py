...
def droid_visualization(video, device="cuda:0"):
    ...
    def animation_callback(vis):
        ...
        with torch.no_grad():
            ...
            points = droid_backends.iproj(SE3(poses).inv().data, disps, video.intrinsics[0], video.model_id).cpu()

            thresh = droid_visualization.filter_thresh * torch.ones_like(disps.mean(dim=[1,2]))
            
            count = droid_backends.depth_filter(
                video.poses, video.disps, video.intrinsics[0], dirty_index, thresh, video.model_id)
            ...
            for i in range(len(dirty_index)):
                ...
                if ix in droid_visualization.cameras:
                    vis.remove_geometry(droid_visualization.cameras[ix], reset_bounding_box=droid_visualization.do_reset)
                    del droid_visualization.cameras[ix]

                if ix in droid_visualization.points:
                    vis.remove_geometry(droid_visualization.points[ix], reset_bounding_box=droid_visualization.do_reset)
